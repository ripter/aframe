...
const { registerElement } = require('../a-register-element');
 ...
 * @member {number} time
 */
class AScene extends HTMLElement {
  constructor(...args) {
    ...
    self.render = bind(this.render, this);
    self.systems = {};
    self.systemNames = [];
    self.time = this.delta = 0;

    self.behaviors = {tick: [], tock: []};
    self.hasLoaded = false;
    self.isPlaying = false;
    self.originalHTML = this.innerHTML;

    // Default components.
    self.setAttribute('inspector', '');
    self.setAttribute('keyboard-shortcuts', '');
    self.setAttribute('screenshot', '');
    self.setAttribute('vr-mode-ui', '');
    return self;
  }
  ...
  */
  getComputedAttribute(attr) {
    ...
    throw new Error('getComputedAttribute is deprecated');
...
registerElement('a-scene', AScene);
